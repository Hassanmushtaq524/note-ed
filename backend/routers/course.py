...
from models import User, Course, Topic, Note
...
@router.post("/")
def add_course(request: Request, data: AddCourseBody, db: Session = db_dependency, user: User = Depends(admin_required)):
    """
    ...
    try:
        ...
        if course:
            return JSONResponse(status_code=400, content={ "detail": "Course already exists" })
...
def get_all_topics(course_id: int, request: Request, db: Session = db_dependency):
    """
    Get all topics for the selected course 
    """
    try:
        ...
        topic_list = db.query(Topic).filter(Topic.course_id == course_id).all()
        return {
            "status": "success",
            "course": course_info,
            "topics": topic_list
...
def add_topic(course_id: int, data: AddTopicBody, request: Request, db: Session = db_dependency, admin: User = Depends(admin_required)):
    """
    Add topic for the selected course id
    """
    try:
        new_topic = Topic(name=data.name, course_id=course_id)
        db.add(new_topic)
        db.commit()
        topic_list = db.query(Topic).filter(Topic.course_id == course_id).all()
        return {
...
def get_all_notes(topic_id: int, request: Request, db: Session = db_dependency):
    """
    Get all notes for selected topic
    """
    try:
        notes_list = db.query(Note).filter(Note.topic_id == topic_id).all()
        return {
            "status": "success",
            "notes": notes_list
        }
...
async def add_note(topic_id: int, file: Annotated[UploadFile, File()], request: Request, db: Session = db_dependency, user: User = Depends(auth_required)):
    """
    ...
    try:
        ...
        new_note = Note(name=file.filename, user_id=user._id, topic_id=topic_id, pdf_url=pdf_url)
        db.add(new_note)
        db.commit()
        db.refresh(new_note)
        return {
